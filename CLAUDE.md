...
- **2026-05-07** — Webhook delivery is queued, not inline-blocking. `create_order` enqueues rows in `webhook_deliveries` atomically with the order; a shared dispatch loop in `supabase/functions/_shared/dispatch.ts` claims rows (`FOR UPDATE SKIP LOCKED` via the `claim_webhook_deliveries` RPC, so concurrent runs never double-send) and POSTs them. Two callers share the loop: the orders function calls it inline (`limit=10`) for first-delivery within the order-create response (initially tried fetching the dispatcher Edge Function via its public URL, but the cold-start latency was 3s+, blowing past any reasonable timeout); the `dispatch-webhooks` Edge Function calls it on a 1-minute pg_cron schedule as the safety net for retries and backlog. Backoff: 30s → 6h × 6 attempts; 4xx is terminal (supplier rejecting payload). Semantics: at-least-once — suppliers must dedupe on `X-Negatic-Delivery`. Cron setup is per-environment (needs project URL + service role key) and lives in `docs/setup/webhook-cron.md` rather than a migration.
